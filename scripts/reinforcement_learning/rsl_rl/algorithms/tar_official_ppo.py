...
# Based on ammousa/TARLoco (CC-BY-NC-SA 4.0).
...
from storage.tar_official_rollout_storage import TAROfficialRolloutStorage


class TAROfficialPPO:
    """PPO + TAR aux loss (pos² + max(0, 1-d_neg) contrastive hinge + vel MSE)."""

    actor_critic: TARActorCriticOfficial
    ...
    def __init__(
        ...
        self.storage: TAROfficialRolloutStorage | None = None
        self.optimizer = optim.Adam(self.actor_critic.parameters(), lr=learning_rate)
        self.transition = TAROfficialRolloutStorage.Transition()
        ...
        print(f"\n[TAR-official] PPO initialized: tar_coef={tar_coef}, vel_coef={vel_coef}")

    def init_storage(self, num_envs, num_transitions_per_env, obs_shape, critic_obs_shape, actions_shape):
        self.storage = TAROfficialRolloutStorage(
            num_envs=num_envs,

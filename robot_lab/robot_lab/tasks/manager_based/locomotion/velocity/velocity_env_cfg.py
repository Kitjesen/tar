...
class CommandsCfg:
    ...
    base_velocity = mdp.UniformThresholdVelocityCommandCfg(
        ...
        debug_vis=False,  # 🔧 禁用调试可视化，避免网络下载问题
...
class CurriculumCfg:
    ...
    disturbance_levels = CurrTerm(func=mdp.disturbance_levels_vel)

    mass_randomization_levels = CurrTerm(func=mdp.mass_randomization_levels_vel)

    com_randomization_levels = CurrTerm(func=mdp.com_randomization_levels_vel)
    ...
    def disable_zero_weight_rewards(self):
        ...
        for attr in dir(self.rewards):
            if not attr.startswith("__"):
                ...
                # Skip callables and already disabled (None) entries
                if reward_attr is None or callable(reward_attr):
                    continue
                # Only disable if this is a reward term with a numeric weight of 0
                weight = getattr(reward_attr, "weight", None)
                if weight == 0:
